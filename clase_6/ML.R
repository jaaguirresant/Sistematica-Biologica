...
# Al igual que la parsimonia, la máxima verosimilitud requiere un árbol inicial para comnezar los análisis. En este caso inferiremos un árbol de distancia con UPGMA y otro con NJ para la exploración de datos y también para usar uno de ellos como árbol inicial en los análisis de Máxima Verosimilitud.

dm <- dist.ml(Primates_ADN) # Generamos la matriz de distancias (corregidas usando el modelo JC69)
arbolUPGMA <- upgma(dm)
arbolNJ <- NJ(dm)
...
plot(arbolUPGMA, main="UPGMA")
plot(arbolNJ, "unrooted", main="NJ")

####################### E. Inferencia de la Máxima Verosimilitud  ######################

# Usaremos dos funciones para las inferencias: "pml", "optim.pml". "pml" simplemente calcula la verosimilitud en un árbol fijo de topología y ramas conocidas usando un modelo especificado por el usuario. Por otro lado, la función "optim.pml" permite optimizar los parámetros (p.e. topología, longitud de ramas, etc) para buscar el árbol con la máxima verosimilitud. 
...
logLik(max_verosim_JC) # veamos el valor de la máxima verosimilitud
...
max_verosim_GTR <- update(verosimilitudNJ, k=4, inv=0.2)
max_verosim_GTR
max_verosim_GTR <- optim.pml(max_verosim_GTR, model="GTR", optInv=TRUE, optGamma=TRUE, rearrangement = "NNI", control = pml.control(trace = 0))
max_verosim_GTR

# o podemos hacer una búsqueda más sofisticada:

max_verosim_GTR <- optim.pml(max_verosim_GTR, model="GTR", optInv=TRUE, optGamma=TRUE, rearrangement = "stochastic", control = pml.control(trace = 0))
max_verosim_GTR
root(max_verosim_GTR$tree,outgroup="Tarsius_syrichta")->arbol_GTR_enraizado
plot(arbol_GTR_enraizado)
